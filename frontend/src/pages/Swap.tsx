...
export default function Swap() {
  ...
  const calculateOutput = useCallback(() => {
    ...
    const rate = fromToken === 'ETH' || fromToken === 'WETH' ? 2100 : 0.000476;
    ...
    try {
      ...
      if (recomputedCommitment.toLowerCase() !== commitmentData.commitment.toLowerCase()) {
        setModal(prev => ({
          ...prev,
          status: 'error',
          error: 'Salt mismatch! Check your saved salt.'
        }));
        setCommitStatus('committed');
        return;
      }

      // CRITICAL FIX: Pool is ETH (currency0) / USDC (currency1)
      // This matches the UI pool configuration exactly
      const poolKey = {
        currency0: TOKENS.ETH.address, // Native ETH (0x0000...)
        currency1: TOKENS.USDC.address, // USDC
        fee: POOL_FEE, // 500
        tickSpacing: POOL_TICK_SPACING, // 10
        hooks: HOOK_ADDRESS,
      };

      // If swapping USDC -> ETH: zeroForOne = false (USDC is currency1)
      // If swapping ETH -> USDC: zeroForOne = true (ETH is currency0)
      const zeroForOne = commitmentData.fromToken === 'ETH' || commitmentData.fromToken === 'WETH';
      ...
      // Price limits based on direction:
      // - zeroForOne=true (ETH->USDC): MIN_SQRT_PRICE + 1
      // - zeroForOne=false (USDC->ETH): MAX_SQRT_PRICE - 1
      const sqrtPriceLimitX96 = zeroForOne
        ? MIN_SQRT_PRICE // Already has +1 in constants
        : MAX_SQRT_PRICE; // Already has -1 in constants
        ...
        <div className="bg-white rounded-3xl p-4 shadow-xl border border-gray-100">
          ...
          <div className="flex justify-between items-center mt-4 mx-2 mb-2">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span>Rate: 1 {getDisplaySymbol(fromToken)} ≈ {fromToken === 'ETH' || fromToken === 'WETH' ? '2,100' : '0.000476'} {getDisplaySymbol(toToken)}</span>
            </div>
                      ...
                      if (manualSalt && commitmentData) {
                        try {
                          ...
                          if (testCommitment.toLowerCase() === commitmentData.commitment.toLowerCase()) {
                            ...
                            alert('❌ Salt does not match commitment.');
                          }
                        } catch {
