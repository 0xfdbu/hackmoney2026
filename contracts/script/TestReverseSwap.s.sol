...
contract TestReverseSwap is Script {
    ...
    // For WETH -> USDC (zeroForOne = false), price goes UP
    // Use MAX_SQRT_PRICE - 1 as limit
    uint160 constant MAX_SQRT_PRICE = 1461446703485210103287273052203988822378723970342;
    
    function run() external {
        vm.startBroadcast(vm.envUint("PRIVATE_KEY"));
        
        // New salt for fresh commitment
        uint256 salt = 1770405996000007;
        uint256 amount = 1000000000000000; // 0.001 WETH (in wei)
        uint256 minOut = 0;
        
        bytes32 commitment = keccak256(abi.encodePacked(amount, minOut, salt));
        bytes32 nullifier = keccak256(abi.encodePacked(salt));
        
        console.log("Amount (WETH wei):", amount);
        ...
        console.log("Revealing and swapping WETH -> USDC...");
        
        // Approve WETH
        (bool a,) = WETH.call(abi.encodeWithSelector(0x095ea7b3, ROUTER, amount));
        ...
        // For zeroForOne = false (WETH -> USDC), price goes UP
        // Use MAX_SQRT_PRICE - 1
